...
public class EscapeSequenceParser {
    ...
    static func buildVt500TransitionTable () -> TransitionTable
    {
        ...
        // Bytes 0x80-0x9f while collecting an OSC/APC string are UTF-8
        // continuation bytes, not C1 controls: the in-chunk fast loop in the
        // .oscPut action already appends them to the payload, but when a feed
        // chunk boundary falls inside a multi-byte character, the first byte
        // of the next chunk goes through this table instead. Without these
        // entries it is misread as a C1 control (e.g. 0x9c — the middle byte
        // of '✳' and friends — terminated the OSC early and spilled the rest
        // of the title string into the screen buffer).
